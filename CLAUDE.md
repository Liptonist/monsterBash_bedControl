...
- Every page's inline `<style>` ends with a `@media (pointer: coarse)` block that sets its form controls to `font-size: 16px` (`.auth-inp`'s copy lives in `AUTH_CSS` in `app-common.js`, since the login overlay is JS-injected). iOS Safari zooms the page in when a field smaller than 16px is focused and does not zoom back out, and it ignores `maximum-scale` in the viewport meta — matching 16px is the only fix that works without taking pinch-zoom away. The blocks name selectors rather than bare `input, select, textarea` because the page's own class rules would otherwise win on specificity, so **a new field needs adding to its page's block by hand**; `index.html`'s also widens `.chart-row`'s first column, which is sized to fit 「サ（サーカス）」.
