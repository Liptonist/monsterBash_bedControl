...
export function fixName(s) {
  ...
  // ひらがな → カタカナ（ぁ-ゖ と 繰り返し記号ゝゞ。長音符「ー」は共通なのでそのまま）
  return t.replace(/[ぁ-ゖゝゞ]/g, c => String.fromCharCode(c.charCodeAt(0) + 0x60));
}

// 氏名に漢字が含まれるか。読みが分からず表記をそろえられないので、
// 入力欄では漢字を弾いてカタカナで入れてもらう（判定はここだけで持つ）。
// 常用漢字（CJK統合漢字）のほか、拡張A・互換漢字・拡張B以降（サロゲートペア）と、
// 「佐々木」の々・〆・〇も対象にする。カタカナ・長音符「ー」は含まれない。
const KANJI_RE = /[\u3005-\u3007\u3400-\u4DBF\u4E00-\u9FFF\uF900-\uFAFF\u{20000}-\u{2FA1F}]/u;
export const hasKanji = s => KANJI_RE.test(String(s ?? ''));
...
export const CHANGELOG = [
  ...
  {version: '1.1.1', date: '2026-08-22', items: [
    '患者IDの付かない退室記録ができてしまうのを直した。「保存する」を押さずに'
      ...
      + '（1日目の記録に、患者IDが「—」の行が混じっているのはこのためです）',
    '空きの枠では「退室する」を押せないようにした。氏名か症状を入れれば押せます'
      + '（受け入れの保存をせずにすぐ帰る人のため）。そのときは確認のうえ、'
      + '患者IDを発番してから記録します',
    '入室中なのに患者IDが付いていない枠（登録したときに通信が切れていた場合など）も、'
      + '退室のときに発番するようにした',
  ]},
  ...
  {version: '1.0.1', date: '2026-08-22', items: [
    '退室したあとの枠に、患者ID・入室時刻・年齢だけが残って見えることがあったのを直した。'
      ...
      + '開き直したときにその3つが消えずに残っていました',
    '残っていたのはその端末の画面上だけで、データベースは正しく空になっています。'
      + 'ただし、その枠に次の人を登録すると前の患者の患者IDを引き継いでしまうため、'
      + '各端末を新しい版に読み込み直してください',
  ]},
  {version: '1.0.0', date: '2026-08-18', items: [
    'MONSTER BaSH 2026（8月22日・23日）の本番版。0.13.2 から動きは変わっていません。'
      + '当日みんなが同じ版を使えるように、版番号を 1.0.0 にしました',
    '古い画面を開いたままの端末や、前に開いたときの画面が残っている端末には、'
      + '上部に青いバーで「新しい版があります（v1.0.0）」と出ます。'
      + '［再読み込み］を押してください。押さなくても、操作していない端末は'
      + '1分ほどで自動的に新しくなります'
      + '（入力中・保存中・編集画面を開いている間は自動では行いません）',
    '本番前に一度、各端末で画面を開き直して、右上の「このアプリについて」で'
      + 'バージョンが 1.0.0 になっているか確かめてください',
  ]},
  {version: '0.13.2', date: '2026-08-13', items: [
    ...
    '編集画面を開いている間にその枠が他の端末で減らされると、保存を押しても'
      + '何も起きなかったのを直した。枠を戻して登録する／退室の記録だけ残す／'
      + '移動先へ入れ直す、を聞くようにしたので、入力した内容は失われません',
    '患者さんが入っている枠を「−」で消せてしまい、入力した記録ごと'
      + '消えることがあったのを直した。いちばん後ろの枠が使用中のあいだ「−」は'
      + '押せなくなり、理由をその場に出す。別の端末がちょうど登録した直後でも、'
      + 'サーバー側で確かめてから減らすので消えない',
    '編集画面の「送迎待ち」の隣に「付き添いあり」のチェックを足した。'
      + 'チェックすると、枠と検索結果に「付添あり」と出ます'
      + '（データベースのルールの更新が必要です）',
    'タブ行のボタンの名前を短くした（退室済・過去・履歴・管理・スリープ防止）。'
      + '正式な呼び名はボタンを長押し／マウスを乗せると出ます。'
      + 'スリープ防止のオン・オフは、これまでの文字ではなくボタンの色で分かります',
  ]},
  {version: '0.13.1', date: '2026-08-13', items: [
    'メイン画面の検索で、氏名をひらがなで打っても見つかるようにした'
      + '（「やまだ」で「ヤマダ タロウ」が出ます）。'
      + '症状は打った文字のまま探すので、これまでどおり漢字でもかなでも探せます',
    '退室済み一覧の氏名検索を、患者名の入力欄と同じ扱いにした。'
      + 'ひらがなはカタカナにそろえ、漢字では探せません',
    '退室済み一覧と保存済みの記録の「CSVで保存」を管理者だけが使えるようにした'
      + '（管理者以外にはボタンを出しません）',
  ]},
  ...
  {version: '0.12.2', date: '2026-08-12', items: [
    '患者名の表記を自動でそろえるようにした。カタカナ表記が基本になり、'
      ...
      + '（例：やまだ　たろう → ヤマダ タロウ）',
    '患者名に漢字が入っていると保存できないようにした。入力欄が赤くなり'
      + '「漢字は使えません。カタカナで入力してください」と出ます'
      + '（打った文字は消えないので、カタカナに直してから保存してください）',
    '漢字のまま登録されている患者の退室と移動は、これまでどおりできます',
    'アルファベットはそのままです。漢字の読みはブラウザでは分からないため'
      + 'カタカナに直せず、表記をそろえられないので入力を止めています',
  ]},
  ...
  {version: '0.10.0', date: '2026-08-07', items: [
    ...
    'メイン画面で患者名や症状の記号がそのまま表示に混ざらないようにした',
    '日次の締めのあと、開いたままの画面が古い患者番号を持ち続けるのを修正',
    'オフライン時の案内を、実際の動作（未送信の操作は失われる）に合わせた',
  ]},
  {version: '0.9.1', date: '2026-08-01', items: [
    '退室済み一覧とゴミ箱に「重症度」の列を追加（軽症・中等症・重症を色付きで表示）',
    'データベースのセキュリティルールに、書き込める内容の検証と削除権限の分離を追加',
  ]},
